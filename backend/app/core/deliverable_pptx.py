...
_SPEAKER_NOTES_CHUNK = 15000


def _chunk_speaker_notes(text: str, chunk_size: int = _SPEAKER_NOTES_CHUNK) -> list[str]:
    """Split speaker notes at paragraph boundaries; never hard-truncate mid-paragraph."""
    body = (text or "").strip()
    if not body:
        return []
...
class PPTXDeliverable(IDeliverable):
    ...
    def _render_with_python_pptx(
        ...
        try:
            ...
            slides = apply_layout_planner_if_enabled(
                slides if isinstance(slides, list) else []
            )
    ...
    def _set_slide_notes(self, slide: Any, text: str) -> None:
        ...
        try:
            ...
            chunks = _chunk_speaker_notes(body)
            tf.clear()
