...
def create_access_token(subject: str) -> str:
    expiry = datetime.now(timezone.utc) + timedelta(minutes=settings.jwt_access_exp_minutes)
...
def create_refresh_token(subject: str, db: Session, user_id: str) -> str:
    jti = str(uuid.uuid4())
    expiry = datetime.now(timezone.utc) + timedelta(days=settings.jwt_refresh_exp_days)
...
def create_sse_token(subject: str, *, run_id: str | None = None) -> str:
    exp = datetime.now(timezone.utc) + timedelta(seconds=max(30, int(settings.jwt_sse_exp_seconds)))
