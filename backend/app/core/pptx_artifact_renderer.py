...
def _render_pptx_with_artifact_tool_impl(
    ...
    try:
        ...
        pptx_slides = apply_layout_planner_if_enabled(
            pptx_slides if isinstance(pptx_slides, list) else []
        )
        source_registry = load_source_registry(payload if isinstance(payload, dict) else {}, run_dir)
        process_model = payload.get("process_model") if isinstance(payload.get("process_model"), dict) else None
        if source_registry and isinstance(pptx_slides, list):
            from app.core.evidence_validator import apply_source_citations_to_slides

            pptx_slides = apply_source_citations_to_slides(pptx_slides, source_registry, process_model)
            if isinstance(payload, dict) and isinstance(payload.get("pptx_slides"), list):
                payload["pptx_slides"] = pptx_slides
        ...
        source_registry = source_registry or load_source_registry(
            payload if isinstance(payload, dict) else {}, run_dir
        )
        ...
        if unsupported_claims > 0:
            ...
            if settings.pptx_evidence_hard_fail_enabled and source_registry:
                ...
                qa_report["evidence_hard_fail"] = True
